...
int carSpeed = 0;							// the speed of the car for easy modification
...
int main() {
	...
	TPM0_C1V = carSpeed;				// TPM0 channel1 left Motor 1 In 1 slow forward
	TPM0_C5V = carSpeed;				// TPM0 channel5 right Motor 2 In 2 slow forward
...
void FTM1_IRQHandler()				// TPM1 ISR
{
	...
	{
		ImageDataDifference.at(i) = abs(ImageData.at(i) - ImageData.at(i + 1));
	}
	CompareData = THRESHOLD;					// threshold
	BlackLineRight = 126;
	for (i = 64; i < 127; i++)
	{
		if (ImageDataDifference.at(i) > CompareData)
		{
			CompareData = ImageDataDifference.at(i);
			BlackLineRight = i;
		}
	}
